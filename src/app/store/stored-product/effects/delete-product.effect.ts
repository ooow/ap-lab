...
import {langSelector} from 'src/app/store/lang/lang.selectors';
...
export class DeleteProductEffect {
  deleteProduct$ = createEffect(
    () => this.actions$.pipe(ofType(deleteProductAction),
      switchMap(({product, lang}) => {
        return this.productService.deleteProduct(product, lang)
          .pipe(map((response) => {
            return deleteProductSuccessAction({product: response.product});
          }), catchError((errorResponse: HttpErrorResponse) => {
            return of(deleteProductFailureAction({error: errorResponse.error}));
          }));
      })));

  updateProductsAfterDelete$ = createEffect(
    () => this.actions$.pipe(ofType(deleteProductSuccessAction),
      withLatestFrom(this.store.pipe(select(langSelector))),
      switchMap(([{product}, lang]) => [
        getTopProductsAction({lang}), getProductsAction({lang, pageIndex: 0}),
      ])));
