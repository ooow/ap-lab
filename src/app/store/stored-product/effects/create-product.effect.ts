...
export class CreateProductEffect {
  createProduct$ = createEffect(
    () => this.actions$.pipe(ofType(createProductAction),
      switchMap(({productData, lang}) => {
        return this.productService.createProduct(productData, lang)
          .pipe(map(({product}) => createProductSuccessAction({product})),
