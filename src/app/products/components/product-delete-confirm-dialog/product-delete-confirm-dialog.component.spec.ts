...
describe('ProductDeleteConfirmDialogComponent', () => {
  ...
  class TestComponent {
    ...
    openDialog(): void {
      const dialogRef = this.dialog.open(ProductDeleteConfirmDialogComponent, {
        data: product
      });
  ...
  beforeEach(async () => {
    ...
    dialogHarness = await rootLoader.getHarness(
      ProductDeleteConfirmDialogHarness
    );
  });

  it('renders component', async () => {
    expect(await (await dialogHarness.getTitle()).text()).toBe(
      'Please confirm'
    );
    expect(await (await dialogHarness.getBody()).text()).toContain(
      product.name
    );
