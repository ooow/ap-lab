...
import {ChartType} from 'src/app/dashboard_view/components/chart/chart.types';
...
describe('Chart', () => {
  ...
  beforeEach(async () => {
    ...
    window['google'] = {
      charts          : {
        load: jasmine.createSpy('load'), setOnLoadCallback: jasmine
        ...
        PieChart                                 : jasmine.createSpy().and
          .returnValue({draw: drawSpy}),
        BarChart                                 : jasmine.createSpy().and
          .returnValue({draw: drawSpy}),
      },
    };
