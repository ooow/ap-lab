...
import {ProductTableComponent} from './product_table';
import {ProductTableHarness} from './testing/product_table_harness';

@Component({
  selector: 'tk-test', template: '<tk-product-table></tk-product-table>',
})
class TestComponent {}
...
describe('ProductTable', () => {
  ...
  const products = [
    {
      name: 'first', picture: 'picUrl', description: 'firstDesc', counts: [
  ...
  beforeEach(async () => {
    ...
    component =
      fixture.debugElement.query(By.css('tk-product-table')).componentInstance;
  ...
  describe('table', () => {
    ...
    it(
      'should call snackBar open method with expected params on Copy Url btn click',
      async () => {
        const harness = await loader.getHarness(ProductTableHarness);
        ...
        expect(fakeSnackBarService.open)
          .toHaveBeenCalledOnceWith(...expectedParams);
      });
    ...
    it('should sort products', async () => {
      ...
      expect((await first.getCellTextByIndex({columnName: 'name'}))[0])
    ...
    describe('delete product button', () => {
      ...
      it('should render delete btn only for products with id field',
        async () => {
      ...
      it('should propagate delete product event', async () => {
        ...
        const deleteBtn = (await harness.deleteProductBtns(
          0)) as MatButtonHarness | null;
        if (deleteBtn) {
          await deleteBtn.click();
        }
        expect(component.deleteProduct.emit)
          .toHaveBeenCalledOnceWith(expectedParams);
      });
    });
    ...
    describe('product details link', () => {
      ...
      it('should propagate product details event', async () => {
        ...
        expect(component.productDetails.emit)
          .toHaveBeenCalledOnceWith(expectedParams);
      });
    });
  });
